...
describe('Mock Client Utilities', () => {
  ...
  describe('MockAdapter basic functionality', () => {
    ...
    it('should mock PUT requests', async () => {
      ...
      const result = await mockAdapter.client.put('/users/1', {
        name: 'Updated',
      })
      expect(result.ok).toBe(true)
      if (result.ok) {
    ...
    it('should mock PATCH requests', async () => {
      ...
      const result = await mockAdapter.client.patch('/users/1', {
  ...
  describe('MockAdapter advanced features', () => {
    ...
    it('should support function-based responses', async () => {
      ...
      if (result.ok) {
        ...
        expect(result.value.data.url).toBe(
          'https://api.example.com/dynamic?page=1&limit=10',
        )
      }
    })
  ...
  describe('MockAdapter utility methods', () => {
    ...
    it('should handle binary data responses', async () => {
      ...
      if (result.ok) {
        ...
        expect(
          result.value.data instanceof Blob ||
    ...
    it('should handle text responses', async () => {
      ...
      const result = await mockAdapter.client.get('/text', {
        responseType: 'text',
      })
      expect(result.ok).toBe(true)
      if (result.ok) {
