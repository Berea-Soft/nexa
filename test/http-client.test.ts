...
describe('HTTP Client Plugin', () => {
  ...
  describe('Basic HTTP Methods', () => {
    ...
    describe('POST requests', () => {
      it('should make a POST request with JSON body', async () => {
        ...
        expect((options as RequestInit).body).toEqual(
  ...
  describe('Interceptors (Open/Closed principle)', () => {
    ...
    it('should support multiple interceptors', async () => {
      ...
      client.addRequestInterceptor({
        onRequest: (req: HttpRequestConfig) => ({
          ...req,
          headers: { ...req.headers, 'X-Custom-1': 'value1' },
        }),
      })
      client.addRequestInterceptor({
        onRequest: (req: HttpRequestConfig) => ({
          ...req,
          headers: { ...req.headers, 'X-Custom-2': 'value2' },
        }),
      })
  ...
  describe('Interceptor Disposal', () => {
    it('should return a disposer that removes the request interceptor', async () => {
      ...
      const headersBefore = (fetchMock.mock.calls[0][1] as RequestInit)
      ...
      dispose()
      await client.get('/test')
      const headersAfter = (fetchMock.mock.calls[1][1] as RequestInit)
        .headers as Record<string, string>
  ...
  describe('Pagination (paginate)', () => {
      ...
      for await (const items of client.paginate<Page>('/items', {
        ...
        getNextPage: (data, cfg) =>
          data.nextCursor
            ? {
                ...cfg,
                query: { ...(cfg.query as object), cursor: data.nextCursor },
              }
            : null,
      })) {
    ...
    it('should stop on API error', async () => {
      ...
      // When: iterating pages
      const pages: unknown[][] = []
      for await (const items of client.paginate<{
        items: string[]
        next: boolean
      }>('/items', {
        getItems: (data) => data.items,
  ...
  describe('Polling (poll)', () => {
    it('should poll until condition is met', async () => {
      ...
      const result = await client.poll<{ status: string; progress: number }>(
        '/jobs/1',
        {
          intervalMs: 10,
          maxAttempts: 10,
  ...
  describe('Response type handling - formData', () => {
    ...
    it('should auto-detect multipart/form-data as formData', async () => {
      ...
      const result = await client.get('/multipart-data', {
        responseType: 'auto',
      })
      ...
      if (result.ok) {
        expect(
          result.value.data instanceof FormData ||
            result.value.data === mockFormData,
        ).toBe(true)
      }
    })
  })
    ...
    it('should send URLSearchParams as body', async () => {
      ...
      expect(
        Object.values(headers).some(
          (h) =>
            typeof h === 'string' &&
            h.includes('application/x-www-form-urlencoded'),
        ),
      ).toBe(true)
    })
  ...
  describe('Response parsing edge cases', () => {
    it('should import and use HttpError from library', async () => {
      ...
      const error = new HttpError('Server error', 500, 'SERVER_ERROR', {
        detail: 'test',
      })
  ...
  describe('Plugin System - Example Plugins Coverage', () => {
    ...
    it('should execute LoggerPlugin request:start handler', async () => {
      ...
      // Then: should log request started (gives coverage of console.warn in handler)
    ...
    it('should execute LoggerPlugin request:success handler', async () => {
      ...
      // Then: should log success (executes console.warn in success handler)
      const calls = consoleSpy.mock.calls.map((c) => c[0])
      expect(
        calls.some(
          (c) => typeof c === 'string' && c.includes('Request succeeded'),
        ),
      ).toBe(true)
