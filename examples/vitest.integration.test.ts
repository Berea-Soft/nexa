...
describe("Nexa HTTP Client - Vitest Integration Examples", () => {
  ...
  describe("Basic Mocking", () => {
    ...
    it("should mock POST requests with request body validation", async () => {
      ...
      // Act
      const result = await mockAdapter.client.post("/users", { body: newUser });
  ...
  describe("Testing Interceptors", () => {
    it("should test request interceptors", async () => {
      ...
      // Act
      await mockAdapter.client.get("/test");
    ...
    it("should test response interceptors", async () => {
      ...
      const interceptor = vi.fn((response) => {
        if (response.ok) {
          response.value.data.modified = true;
        }
        return response;
      });
      ...
      mockAdapter.onGet("/data").reply(200, { original: true });

      // Act
      const result = await mockAdapter.client.get("/data");
  ...
  describe("Testing Retry Strategies", () => {
    it("should test retry logic with mock failures", async () => {
      ...
      let callCount = 0;
      mockAdapter.onGet("/retry").reply(() => {
        callCount++;
        if (callCount === 1) {
          return { status: 500, data: { error: "Temporary failure" } };
        }
        return { status: 200, data: { success: true } };
      });

      // Create a client with aggressive retry strategy
      const retryClient = createHttpClient({
        baseURL: "https://api.example.com",
        retry: {
          maxAttempts: 3,
          backoff: () => 0, // No delay for testing
        },
      });
      const retryMock = createMockClient(retryClient);

      // Use the same mock route
