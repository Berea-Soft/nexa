...
interface MockRoute {
  ...
  response:
    | MockResponse
    | ((config: HttpRequestConfig) => MockResponse | Promise<MockResponse>)
...
export class MockAdapter {
  ...
  constructor(client: IHttpClient, options: MockClientOptions = {}) {
    ...
    if (
      typeof (client as unknown as Record<string, unknown>).extend ===
      'function'
    ) {
      this.mockClient = (
        client as unknown as {
          extend: (config: { adapter: typeof adapter }) => IHttpClient
        }
      ).extend({ adapter })
    } else {
      ...
      throw new Error(
        'MockAdapter requires an HttpClient instance with extend() method',
      )
    }
  }
  ...
  createAdapter(): (
    ...
    return async (
      ...
      if (!route) {
        ...
        return new Response(
          JSON.stringify({ error: 'No mock route matched' }),
          {
            status: 404,
      ...
      const userProvidedContentType =
        responseConfig.headers && 'content-type' in responseConfig.headers
      if (!userProvidedContentType) {
        // Set appropriate content-type based on data type
        if (
          finalResponse.data instanceof Uint8Array ||
          ArrayBuffer.isView(finalResponse.data)
        ) {
  ...
  private findMatchingRoute(method: string, url: string): MockRoute | null {
    ...
    for (const route of this.routes) {
      ...
      if (typeof route.urlPattern === 'string') {
        ...
        matches =
          normalizedUrl === route.urlPattern ||
          normalizedUrl.startsWith(route.urlPattern)
